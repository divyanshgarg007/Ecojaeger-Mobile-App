...
export default function ProductItem(props) {
  ...
  const getPrice = () => {
    let p = 0
    if (!(props?.item?.quantityRangeData?.length > 0 && props?.item?.quantityRangePriceData?.length > 0)) {
      p = getPriceCommon(props?.item?.defaultPerPriceData)
    } else {
      p = getFactoryPrice(factoryUnit)?.replace('CHF', '')
    }

    return getFormattedPrice(parseFloat(price))
  }
  ...
  return (
    <View style={styles.itemCard}>
      <View style={styles.itemInfo}>
        ...
        {authState?.signIn?.data?.data?.tokenNumber &&
          <Pressable style={styles.addBuying} onPress={() => network?.isConnected ? props.handleAddBuying(props?.item?.id, unitPrice?.id) : alert(strings.common.offlineMessage)} >
            <FastImage
              style={styles.productAdd}
              resizeMode="contain"
              source={StarIcon} />
